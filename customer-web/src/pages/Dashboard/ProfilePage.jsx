...
export const ProfilePage = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {/* Main Card */}
      <div className="bg-white border border-slate-200 rounded-xl p-6 md:p-8 space-y-6 shadow-card">
        
        {/* User Role Banner */}
        <div className="flex items-center space-x-4 p-4 rounded-xl bg-slate-50 border border-slate-200">
          <div className="w-12 h-12 rounded-xl bg-orange-500/10 border border-orange-500/30 flex items-center justify-center text-orange-600 font-extrabold text-lg">
            {user?.name?.charAt(0).toUpperCase()}
          </div>
          <div>
            <h3 className="text-lg font-bold text-slate-900">{user?.name}</h3>
            <p className="text-xs text-slate-500">{user?.email}</p>
            <span className="inline-block mt-1 text-[10px] font-extrabold uppercase px-2 py-0.5 rounded bg-orange-500/10 text-orange-600 border border-orange-500/20">
        ...
        {success && (
          <div className="p-3.5 rounded-lg bg-emerald-50 border border-emerald-500/30 text-emerald-700 text-xs font-semibold flex items-center space-x-2">
        ...
        {error && (
          <div className="p-3.5 rounded-lg bg-rose-50 border border-rose-500/30 text-rose-700 text-xs font-semibold">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div className="pt-4 border-t border-slate-200 flex justify-end">
            <button
              type="submit"
              disabled={loading}
              className="flex items-center space-x-2 px-6 py-3 rounded-lg bg-orange-500 hover:bg-orange-600 text-white font-extrabold text-sm transition-all disabled:opacity-50"
            >
