  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res?.data?.role !== 'Customer') {
        logout();
        setError('This portal is for customers only. Staff, Admin and Technician users must use the Management or Technician portal.');
        setLoading(false);
        return;
      }
    ...
    <div className="relative min-h-[80vh] flex items-center justify-center px-4 py-12 overflow-hidden bg-gradient-to-b from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-950">
      ...
      >
        ...
        <div className="text-center space-y-2">
          ...
          <h2 className="text-2xl font-black text-slate-900 dark:text-white font-display">Customer Sign In</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">Track service bookings, invoices, orders & online payments</p>
        </div>
        ...
        <div className="text-center pt-2">
          <p className="text-xs text-slate-500 dark:text-slate-400">
            ...
            <Link to="/register" className="font-bold text-orange-500 hover:text-orange-600 hover:underline">
              Create New Account
            </Link>
          </p>
        </div>
