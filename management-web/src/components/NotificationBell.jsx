...
const NOTIFICATION_ROUTES = {
  ...
  Service: '/dashboard/technician-requests',
  StockAlert: '/dashboard/inventory',
  General: null
};
  ...
  return (
    <div className="relative" ref={rootRef}>
      <button
        ...
        aria-expanded={open}
        aria-haspopup="true"
        className={`relative p-2.5 rounded-xl border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/60 ${
          open
            ...
            <div className="flex-1 min-h-0 overflow-y-auto divide-y divide-slate-100 overscroll-contain">
              {isLoading ? (
                ...
                <div className="p-6 flex flex-col items-center text-center space-y-3">
                  <AlertTriangle className="w-8 h-8 text-rose-600/80" />
                  <p className="text-slate-500 font-medium">Unable to load notifications.</p>
                  <button
                    type="button"
                    onClick={() => refetch()}
                    className="inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-lg bg-white border border-slate-200 text-slate-600 font-bold text-[11px] hover:bg-slate-50 hover:border-slate-300 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/60"
                  >
