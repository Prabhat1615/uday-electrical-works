  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          <h1 className="text-2xl font-extrabold text-white">Enterprise Reporting & Data Export</h1>
          <p className="text-xs text-slate-400">Generate compliance ledgers, inventory valuation, technician performance & CSV exports</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="p-12 text-center bg-slate-900 rounded-2xl border border-slate-800 text-slate-400">
          <BarChart3 className="w-10 h-10 text-amber-500/50 mx-auto mb-2" />
          <h3 className="text-base font-bold text-white">No report data generated</h3>
        </div>
      ) : (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
          <div className="p-4 bg-slate-950 border-b border-slate-800 flex justify-between items-center">
            <h3 className="font-bold text-white text-sm capitalize">{reportType} Detailed Ledger</h3>
            <span className="text-xs text-slate-400 font-semibold">{data.length} Total Records</span>
          </div>
                ...
                {data.map((row, idx) => (
                  <tr key={idx} className="hover:bg-slate-800/40 transition-colors">
                    {Object.values(row).slice(0, 7).map((val, i) => (
                      <td key={i} className="px-6 py-4 truncate max-w-xs font-semibold">
