  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900">Multi-Warehouse & Stock Transfers</h1>
          <p className="text-xs text-slate-500">Manage central storage hubs, regional stock points & inter-location material transfers</p>
        </div>

        <div className="flex items-center space-x-3">
          <button
            onClick={() => setTransferModalOpen(true)}
            className="flex items-center space-x-1.5 px-3.5 py-2 rounded-xl bg-white border border-slate-200 text-slate-600 text-xs font-semibold hover:bg-slate-50"
          >
      ...
      {loadingWh ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {warehouses.map((w) => (
            <div key={w._id} className="bg-white border border-slate-200 rounded-xl p-5 space-y-3 shadow-card relative overflow-hidden">
              <div className="flex justify-between items-start">
                <div>
                  <span className="text-[10px] font-mono font-bold text-orange-600 uppercase tracking-widest">{w.code}</span>
                  <h3 className="text-lg font-bold text-slate-900 mt-0.5">{w.name}</h3>
                </div>
      ...
      <Modal isOpen={whModalOpen} onClose={() => setWhModalOpen(false)} title="Register Warehouse Location">
        <form onSubmit={handleCreateWh} className="space-y-4 text-xs">
          <div>
            <label className="block text-slate-600 font-bold uppercase mb-1">Warehouse Name *</label>
            <input
              type="text"
              required
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-slate-600 font-bold uppercase mb-1">Code (Unique) *</label>
              <input
                type="text"
                required
                placeholder="WH-001"
            ...
            <div>
              <label className="block text-slate-600 font-bold uppercase mb-1">Capacity Units</label>
              <input
                type="number"
                value={capacity}
          ...
          <div>
            <label className="block text-slate-600 font-bold uppercase mb-1">Address / Location *</label>
            <input
              type="text"
              required
      ...
      <Modal isOpen={transferModalOpen} onClose={() => setTransferModalOpen(false)} title="Inter-Warehouse Stock Transfer">
        <form onSubmit={handleTransfer} className="space-y-4 text-xs">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-slate-600 font-bold uppercase mb-1">Source Warehouse *</label>
              <select
                value={fromWhId}
            ...
            <div>
              <label className="block text-slate-600 font-bold uppercase mb-1">Destination Warehouse *</label>
              <select
                value={toWhId}
          ...
          <div>
            <label className="block text-slate-600 font-bold uppercase mb-1">Transfer Quantity *</label>
            <input
              type="number"
              min="1"
              required
