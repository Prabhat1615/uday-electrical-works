    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {tickets.map((t) => (
            <div key={t._id} className="bg-white border border-slate-200 rounded-xl p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-card hover:border-amber-500/40 transition-colors">
              <div className="space-y-1">
                <div className="flex items-center space-x-2">
                  <span className="font-mono text-amber-600 font-bold text-xs">{t.ticketNumber}</span>
              ...
              <div className="flex items-center space-x-3 shrink-0">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-white border border-slate-200 hover:bg-slate-100 text-slate-600 text-xs font-semibold"
                >
                  View & Reply Thread
                </button>
              </div>
        ...
        {activeTicket && (
          <div className="space-y-4 text-xs">
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
              <span className="font-bold text-amber-600 uppercase text-[10px]">{activeTicket.category}</span>
              ...
              {activeTicket.replies?.length === 0 ? (
                ...
                activeTicket.replies.map((r, idx) => (
                  <div key={idx} className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
                    ...
                    <p className="text-slate-900">{r.message}</p>
                  </div>
                ))
              )}
            </div>
            ...
            <form onSubmit={handleReplySubmit} className="space-y-3 pt-3 border-t border-slate-200">
              ...
              <div className="flex justify-end space-x-3 pt-2">
                ...
                <button
                  type="submit"
                  className="flex items-center space-x-2 px-5 py-2 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-extrabold shadow-md"
                >
